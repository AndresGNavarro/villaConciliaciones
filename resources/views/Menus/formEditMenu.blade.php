        ...
        <div class="card shadow col-md-12">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="mb-0">Editar Opción de menú</h3>
                            </div>
                    ...
                    <div class="card-body">
                        <form action="{{ url('/menu/edit/'.$menu->pkMenu) }}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="form-group col-xs-12 col-sm-6 col-md-3">
                                    ...
                                    <input type="text" name="icon" class="form-control @error('icon') is-invalid @enderror" value="{{old('icon',$menu->icon)}}">
                                    @error('icon')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group col-xs-12 col-sm-6 col-md-3">
                                    ...
                                    <option {{ $menu->parent==$row->pkMenu?'selected':''}} {{ old('parent')==$row->pkMenu?'selected':'' }} value="{{$row->pkMenu}}"> 
                                        {{$row->name}} </option>
                                    @endforeach
                                    </select>
                                    @error('parent')
                            ...
                            <div class="row">
                                <div class="form-group col-xs-12 col-sm-6 col-md-3">
                                    ...
                                    <input class="form-control" type="number" value="{{old('order', $menu->order)}}" name="order" id="order" min="0" max="100" required>
                                </div>
                                <div class="form-group col-xs-12 col-sm-6 col-md-3">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input" name="enabled" id="enabled" 
                                        {{ is_null($menu->enabled) || ($menu->enabled==1 ||old('enabled')=='on') ?'checked':'' }}>
