        ...
        <div class="card shadow">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            <div class="col">
                                <i class="fas fa-user-plus fa-2x"></i> <h3 class="mb-0">Nuevo Usuario</h3>
                            </div>
                        ...
                        <form action="/user/create" method="post">
                            ...
                            <div class="row">
                                <div class="form-group col-xs-12 col-sm-4 col-md-4">
                                    ...
                                    <option {{ old('role')==$role->pkRole?'selected':'' }} value="{{$role->pkRole}}"> {{$role->description}} </option>
                                    @endforeach
                                    </select>
                                    @error('role')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="form-group col-xs-12 col-sm-4 col-md-4">
                                    ...
                                    <option {{ old('sucursal')==$subsidiary->pkSubsidiary?'selected':'' }} value="{{$subsidiary->pkSubsidiary}}"> {{$subsidiary->description}} </option>
                                    @endforeach
                                    </select>
                                    @error('sucursal')
                                        <div class="invalid-feedback">{{ $message }}</div>
